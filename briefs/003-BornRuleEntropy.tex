...
\begin{abstract}
	We show that the Born rule in quantum mechanics coincides with the definition of entropy. That is, assuming that conditional probability is given by the Born rule is equivalent to assuming that the entropy is given by the von Neumann entropy.
\end{abstract}
...
\end{equation}
defines conditional probability for two states: the probability of measuring state $\psi$ given that $\phi$ was prepared. Similarly, the von Neumann entropy
\begin{equation}
	S(\rho) = - \tr(\rho \log \rho)
\end{equation}
...
\begin{proof}
	...
	\end{figure}
	The relationship is plotted in Fig. \ref{fig_rp_qm_entropyVsProb}: it is strictly decreasing and, therefore, invertible. If we are given the entropy of all equal mixtures of pairs of states we would be able to recover the Born rule and vice-versa.
\end{proof}
