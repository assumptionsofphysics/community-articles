...
\begin{proof}
	...
	\ref{DR-EV} $\implies$ \ref{DR-UNIT}. By \ref{DR-EV}, $\Phi_t$ maps pure states bijectively onto pure states. By \ref{[B]EVMIX}, it is continuous and affine. Proposition \ref{AFF-PURE} shows that each $\Phi_t$ is implemented by either a unitary or an anti-unitary operator. The anti-unitary case is excluded by composition. In fact, $\Phi_t=\Phi_{t/2}\circ \Phi_{t/2}$, and the composition of two maps implemented by an anti-unitary is implemented by a unitary. Therefore every $\Phi_t$ is implemented by a unitary.

	The composition law gives a continuous projective unitary representation of elapsed time. The phases can be chosen so that $U_0=I$ and $U_{t+s}=U_tU_s$. Joint continuity of $\Phi_t$ gives strong continuity of $U_t$. The inverses define $U_{-t}=U_t^\dagger$, yielding a strongly continuous one-parameter unitary group. Therefore \ref{DR-UNIT} holds.
