...
\tags{Superposition, Mixed states}
...
The first condition states that one state can be expressed as a superposition of two other states.

\begin{condition}[Superposition of pure states]{QSUP-PUR}
	A pure state $\phi$ can be expressed as a superposition of $\psi_1$ and $\psi_2$. That is, $|\phi\> = c_1 |\psi_1\> + c_2 | \psi_2\>$ with $|\psi_1\>, |\psi_2\>$ linearly independent and $c_1,c_2\neq 0$.
\end{condition}
...
\begin{condition}[Multiple decomposition of mixtures]{QSUP-MIX}
	There exists a mixed state $\rho$ that is a mixture of $\psi_1$ and $\psi_2$ and also a mixture of $\phi$ and some other state. That is, $\rho = p_{1} \rho_{\psi_1} + p_{2} \rho_{\psi_2} = p_{\phi} \rho_{\phi} + p_{\hat{\phi}} \rho_{\hat{\phi}}$ with $\rho_{\phi} \notin \{\rho_{\psi_1}, \rho_{\psi_2} \}$, $p_{1},p_{2},p_{\phi},p_{\hat{\phi}} \in (0,1)$ and $p_{1}+p_{2}=p_{\phi}+p_{\hat{\phi}}=1$.
\end{condition}
...
\begin{demonstration}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	Note that $p_{\phi} + p_{\hat{\phi}}=1$, which means $\rho$ is a statistical mixture with the given probabilities. This also means that $p_1 + p_2 = 1$ as well. We therefore have a mixed state that can be expressed either as a mixture of $\psi_1$ and $\psi_2$ or of $\phi$ and $\hat{\phi}$.
...
Reducing pure state superpositions to properties of statistical mixtures means reducing an abstract mathematical property to a physically and operationally well-defined one. Therefore, it can provide more insights into the physics described by the math.
