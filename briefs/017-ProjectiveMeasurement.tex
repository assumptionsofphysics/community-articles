...
\begin{condition}[Closest commuting ensemble]{EQ-HS}
	Given an observable $X=\sum_x xP_x$, the output of the black-box process $\Phi_X$ is the ensemble commuting with $X$ that is closest to the incoming ensemble in Hilbert--Schmidt distance. That is, $\Phi_X(\rho)=\operatorname*{argmin}\limits_{\sigma\in\mathcal D(\mathcal H),\,[\sigma,X]=0}\|\rho-\sigma\|^2_{HS}$.
\end{condition}

\begin{condition}[Closest commuting ensemble in information]{EQ-KL}
	Given an observable $X=\sum_x xP_x$, the output of the black-box process $\Phi_X$ is the ensemble commuting with $X$ that is closest to the incoming ensemble in relative entropy. That is, $\Phi_X(\rho)=\operatorname*{argmin}\limits_{\sigma\in\mathcal D(\mathcal H),\,[\sigma,X]=0}D(\rho\Vert\sigma)$.
\end{condition}
