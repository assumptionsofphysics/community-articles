...
\begin{abstract}
	We characterize a projective measurement as a black-box process that returns the mixed state associated with the a priori post-measurement state. We show that this process can be understood as equilibration under a Lindblad equation or as selection of the closest ensemble commuting with the measured observable. In finite dimension, closeness can be expressed either through Hilbert--Schmidt distance or relative entropy.
\end{abstract}
...
Black-box processes preserve statistical mixtures. This condition is not actually used as it is an automatic consequence of all other conditions. We leave it as a description of what we mean by black-box process.
...
\begin{thrm}
	Over \ref{QST:H}, conditions \ref{EQ-MEAS}, \ref{EQ-LIND} and \ref{EQ-HS} are equivalent. Over \ref{QST:H}+\ref{[B]BBMIX}+\ref{FD}, condition \ref{EQ-KL} is also equivalent to the others.
\end{thrm}
...
\begin{proof}
	...
	\ref{EQ-LIND} $\iff$ \ref{EQ-MEAS}. Since $P_x$ are the projectors for the eigenspaces associated to each $x$, we can write the density matrix as
	\begin{equation}
...
\textbf{Observables with continuous spectra.} Note that all four conditions fail when $X$ has a continuous spectrum, or, in over those parts of the spectrum that are continuous if both types are present. In fact, the pinching map cannot be defined; an operator $\rho$ that commutes with $X$ also has continuous spectrum (i.e. it would have to be multiplicative in the wave-function representation of the observable) and therefore is not trace-class; similarly, fixed points of the Lindblad equation would not be density operators. In other words, there are no equilibria and equilibration processes in that case.
...
More in general, the black-box idea could be extended to destructive measurements. One can imagine a black-box process in which the input is the state of the system while the output is the final state of the measurement device.
