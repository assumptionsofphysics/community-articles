...
\begin{proof}
	...
	\end{equation}
	Each $\Psi_\tau$ is again state-adaptive projective. For fixed $t$ and $\tau=t/n$, Proposition \ref{INF-PRED} shows that the accumulated state error and the total failure probability are both $O(t\tau M_\tau^2/\hbar^2)$, which tends to zero by \ref{SLOW-CUTOFF}.
