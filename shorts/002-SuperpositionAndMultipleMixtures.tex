...
\usepackage{../common/assumptionsofphysics}
...
\author{Gabriele Carcassi and the AoP collaboration}
\date{}
...
\makebox[\textwidth][c]{
	\begin{tabular}[t]{b{15cm} m{2.5cm}}
		\begin{tabular}{@{}l@{}}{\Large\fontfamily{phv}\selectfont\bfseries{Assumptions of Physics}} \\  \url{https:\\assumptionsofphysics.org} \\ \end{tabular}
		& \includegraphics[width=2.5cm] {../common/AssumptionsLogo1000.png}\\
	\end{tabular}}
...
\begin{abstract}
	We show that the possibility of superpositions in quantum mechanics coincides with the possibility of multiple decompositions.
\end{abstract}
	
\section{Introduction}

Given that the addition of state vectors in quantum mechanics does not represent a physically realizable operation, it is difficult to give a physically meaningful direct interpretation to quantum superpositions. Here we show that the existence of superpositions is equivalent to the existence of mixtures with multiple decompositions in terms of pure states. Given that such multiple decompositions are not possible in classical mechanics, this also tells us why classical superpositions are not possible as well.

\section{Premises}

Let us first describe the relevant premises. The base premise is that we are working with quantum states with their standard representation in terms of complex vector spaces.

\begin{premise}[QST]\label{QST}
	The state of quantum system is represented by a ray of the complex inner product space $\mathcal{H}$. An ensemble is represented by a density operator $\rho : \mathcal{H} \to \mathcal{H}$, that is a positive semi-definite trace one self-adjoint operator.
\end{premise}

The first premise states that one state can be expressed as a superposition of other two.

\begin{premise}[QSUP-PUR]\label{QSUP-PUR}
	A pure state $\phi$ can be expressed as a superposition of $\psi_1$ and $\psi_2$. That is, $|\phi\> = c_1 |\psi_1\> + c_2 | \psi_2\>$.
\end{premise}

The second premise states that we can find a mixed state $\rho$ that can be expressed as a mixture of $\psi_1$ and $\psi_2$ as well as a mixture of $\phi$ and something else.

\begin{premise}[QSUP-MIX]\label{QSUP-MIX}
...
The claim is that the two premises are equivalent over the base premise.

\begin{claim}
	The premises \ref{QSUP-PUR} and\ref{QSUP-MIX} are equivalent over \ref{QST}.
\end{claim}
